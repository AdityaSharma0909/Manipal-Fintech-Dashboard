...
from insurance.coverfox.insurance_services import get_medibuddy_sso_url
from insurance.coverfox.serializers import MediBuddySerializer
...
class MediBuddyView(APIView):
    permission_classes = []
    @extend_schema(request=MediBuddySerializer)
    def post(self, request):
        serializer = MediBuddySerializer(data=request.data)
        ...
        try:
            ...
            sso_url = get_medibuddy_sso_url(mobile_number,employee_id)
            if not sso_url:
                return build_response(
                    False,
            ...
            logger.exception("Unexpected error in MediBuddyView.post: %s", exc)
            return build_response(
                False,
