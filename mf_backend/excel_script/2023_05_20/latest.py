...
excel_file = pd.ExcelFile('./excel_script/2023_05_20/latest.xlsx')
...
def upload_excel_to_db_users():
    ...
    for index, each_phone_no in enumerate(all_phone_nos):
        with transaction.atomic():
            ...
            employee_id = utils.clean_str_field(df_employee["Employee ID"][index]).zfill(5)
            user_data = {
...
def upload_excel_to_db_branch():
    ...
    for index, each_branch_code in enumerate(all_branch_code):
        with transaction.atomic():
            branch_data = {
                    "branch_code":utils.clean_str_field( df_branch[" Branch code"][index] ),
...
#exec(open('excel_script/2023_05_20/latest.py').read())