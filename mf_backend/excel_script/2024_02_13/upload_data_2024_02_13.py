...
excel_file = pd.ExcelFile("./excel_script/2024_02_13/sheet2.xlsx")
sheet1 = "Sheet1"
# sheet2 = "CPC Team "
...
class UploadExcelToDB:
    ...
    def identify_role_by_permission(self, raw_role, access, permission):
        processed_access = self.clean_str_field(access).lower()
        processed_permission = self.clean_str_field(access).lower()
        ...
        if processed_access == "los":
            output_role = ROLES.LOAN_OFFICER.value
        elif processed_access == "crm" and processed_permission == "view only":
            output_role = ROLES.BUSINESS_HEAD.value
        elif processed_access == "crm" and processed_permission == "all":
            output_role = self.replace_role(raw_role)
        else:
            output_role = ROLES.BUSINESS_HEAD.value
    ...
    def replace_role(self, raw_role):
        ...
        if "marketing" in processed_role and "sales" in processed_role:
            output_role = ROLES.BUSINESS_HEAD.value
        elif "branch operation manager" in processed_role:
            output_role = ROLES.BRANCH_MANAGER.value
        elif "assistant" in processed_role and "branch" in processed_role:
            output_role = ROLES.BRANCH_MANAGER.value
        elif "cpc" in processed_role:
            output_role = ROLES.CPC.value
        elif "cluster" in processed_role or "city head" in processed_role:
...
def upload_excel_to_db_users():
    ...
    for index, emp_id in enumerate(all_emp_ids):
        with transaction.atomic():
            ...
            user_data = {
                "phone": phone,
                "role": utils.identify_role_by_permission(
                    df_employee["Designation"][index],
                    df_employee["Access"][index],
                    df_employee["Premission"][index],
                ),
...
# errorBM = set(utils.script_stats['new_users_data']) - set(utils.script_stats['new_branch_mapping_data'])
# errorBM2 = set(utils.script_stats['new_branch_mapping_data']) - set(utils.script_stats['new_users_data'])

# print("errorBM: ", errorBM)
# print("\n\n")
# print("errorBM2: ", errorBM2)


"""
python3 manage.py shell
exec(open('excel_script/2024_02_13/upload_data_2024_02_13.py').read())
"""
...
# ===== user_ser_error =====
# {'+919573376802': {'phone': [ErrorDetail(string='user with this phone already exists.', code='unique')]}, '+919573841895': {'phone': [ErrorDetail(string='user with this phone already exists.', code='unique')]}}
# ===== branch_ser_error =====
# {}
# ===== branch_user_mapping_ser_error =====
# {'+919573376802': {'user': [ErrorDetail(string='This field may not be null.', code='null')]}, '+919573841895': {'user': [ErrorDetail(string='This field may not be null.', code='null')]}}
# ===== script_stats =====
# {'user_created': 20, 'user_ser_error': 2, 'updated_user': 0, 'updated_branch': 0, 'branch_created': 0, 'branch_ser_error': 0, 'branch_user_mapping_created': 20, 'branch_user_mapping_ser_error': 2, 'bum_no_branch_id_deteted': 0}
