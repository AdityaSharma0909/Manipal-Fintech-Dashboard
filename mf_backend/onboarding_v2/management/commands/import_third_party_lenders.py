...
COLUMN_MAP = {
    "bank_name": ["bank_name", "bank name", "bankname", "lender_name", "lender name", "name"],
...
class Command(BaseCommand):
    help = "Bulk import Third Party Lenders from a CSV or Excel (.xlsx) file"

    def add_arguments(self, parser):
        parser.add_argument(
            "file_path",
            type=str,
            help="Path to the CSV or Excel (.xlsx) file",
        )
        parser.add_argument(
            "--sheet",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        for idx, row in df.iterrows():
            ...
            try:
                _, was_created = ThirdPartyLender.objects.get_or_create(
                    bank_name=bank_name,
                    ifsc_code=ifsc_code,
